...
public class UserController {
    ...
    @SysLog("跳转系统物料列表页面")
    public String inventList(){
        return "admin/user/inventList";
    }
    ...
    @RequiresPermissions("inv:inventTable:edit")
    ...
    public ResponseEntity editInvent(@RequestBody  InventTable inventtable){
        ...
        if(StringUtils.isBlank(inventtable.getColorName())){
            return ResponseEntity.failure("颜色不能为空");
        }

        if(StringUtils.isBlank(inventtable.getMaterial())){
            return ResponseEntity.failure("材质不能为空");
        }

        inventtableService.updateInvent(inventtable);
